...
RUN sed -ri 's/^display_errors\s*=\s*Off/display_errors = On/g' /vesta-start/etc/php5/apache2/php.ini && \
    ...
    sed -i "s/max_input_time = 60/max_input_time = 3600/" /vesta-start/etc/php5/fpm/php.ini && \
    sed -i "s/max_execution_time = 30/max_execution_time = 3600/" /vesta-start/etc/php5/fpm/php.ini && \
